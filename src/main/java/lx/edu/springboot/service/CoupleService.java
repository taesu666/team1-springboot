...
public class CoupleService {
    ...
    public CoupleResultVO analyzeCouple(String userName, String userBirth, String userGender, String userTime,
        ...
        String prompt = String.format("%s,%s, %s, 태어난시간 %s 출생과 %s,%s, %s, 태어난시간 %s 출생의 두사람을 생년월일과 이름 기준 궁합풀이 해줘. "
                ...
                + "연애운과 동업운 값은 '점수'를 빼고, 텍스트 설명만 JSON에 포함시켜.너지금 계속 68점만나오는데 좀더 진지하게 분석해 제발"
                +"\"정태수_김준하_궁합\": {\r\n"
                + "    \"종합_궁합_점수\": 75,\r\n"
                ...
                + "} 그리고 너무 궁합을 좋게줘 냉정하게 분석해서 줘도돼 진짜 궁합사주를 보는거니까 좀더 잘 분석해줘 무조건 이렇게 줘 무조건 지금 점수가 다 62 점으로 나오는데 100점에 0점 중에서 ㅝ",
